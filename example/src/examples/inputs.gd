...
func _on_GetControllers_pressed() -> void:
	...
	$Frame/Main/Output.append_bbcode("Steam found "+str(STEAM_CONTROLLERS.size())+" connected controllers:\n")
	for CONTROLLER in STEAM_CONTROLLERS.size():
		$Frame/Main/Output.append_bbcode(str(STEAM_CONTROLLERS[CONTROLLER])+"\n")


# Get the input's type by handle
func _on_GetName_pressed() -> void:
	$Frame/Main/Output.append_bbcode("[Steam Inputs] Get input type by it's handle...\n\n")

	for CONTROLLER in STEAM_CONTROLLERS.size():
		var TYPE: String = Steam.getInputTypeForHandle(STEAM_CONTROLLERS[CONTROLLER])

		# Print it to the output
		$Frame/Main/Output.append_bbcode("For handle "+str(STEAM_CONTROLLERS[CONTROLLER])+": "+str(TYPE)+"\n")
...
func _on_Init_pressed() -> void:
	...
	# Print the list to output
	$Frame/Main/Output.append_bbcode("Godot found "+str(GODOT_CONTROLLERS.size())+" connected controllers:\n")
	for CONTROLLER in GODOT_CONTROLLERS:
		$Frame/Main/Output.append_bbcode(str(Input.get_joy_name(GODOT_CONTROLLERS[CONTROLLER]))+"\n")

	# Initialize Steam Inputs
	if Steam.inputInit():
		$Frame/Main/Output.append_bbcode("\nSteam Inputs is running!\n\n")
	else:
...
func _on_Joy_Connection_Changed(device_id: int, connected: bool) -> void:
	if connected:
		$Frame/Main/Output.append_bbcode(str(Input.get_joy_name(device_id))+"\n\n")


# Create a haptic pulse
func _on_Haptic_pressed() -> void:
	$Frame/Main/Output.append_bbcode("[Steam Inputs] Sending haptic pulse to all applicable and connected controllers...\n\n")
...
func _on_HapticRepeated_pressed() -> void:
	$Frame/Main/Output.append_bbcode("[Steam Inputs] Sending repeated haptic pulse to all applicable and connected controllers...\n\n")
...
func _on_Shutdown_pressed() -> void:
	if Steam.inputShutdown():
		$Frame/Main/Output.append_bbcode("[Steam Inputs] Shutdown successfully.\n\n")
	else:
		$Frame/Main/Output.append_bbcode("[Steam Inputs] Shutdown failed for some reason.\n\n")


# Vibrate all connected input handles
func _on_Vibrate_pressed() -> void:
	$Frame/Main/Output.append_bbcode("[Steam Inputs] Vibrating all applicable and connected controllers...\n")

	for CONTROLLER in STEAM_CONTROLLERS.size():
		$Frame/Main/Output.append_bbcode("[Steam Inputs] Vibrating controller "+str(STEAM_CONTROLLERS[CONTROLLER])+" with speeds 1000\n")
